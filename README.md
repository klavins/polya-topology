...
**Mathlib's topology is not imported.** Nothing under `Mathlib.Topology.*` is in the fence, so no
problem can be closed by finding the Mathlib lemma that already knows the answer. A student
defines the apparatus and proves its properties from the real numbers, sets, and the language of
real vector spaces.

The names, though, are Mathlib's. `TopologicalSpace`, `MetricSpace`, `IsOpen`, `closure`,
`frontier`, `Continuous`, `IsCompact` and the rest resolve inside the subject — to the student's
own definitions, not to Mathlib's — so that what is built here is recognised later when Mathlib's
topology *is* imported. Where Mathlib separates a property of a set from the same property of a
space, so does this subject: `IsCompact` and `IsPreconnected` are properties of a set,
`IsCompactSpace` and `IsPreconnectedSpace` of a space. The two structures stay structures, passed
by hand as `T.IsOpen U`, because a single type here carries several topologies at once.
